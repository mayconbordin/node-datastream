...
// This is a variation of the lossy algorithm that does not store the delta values
// for each item, but uses a single delta value.
//
...
LossyCountingVariant.prototype = {
    ...
    getTopK: function(k) {
        if (this.s > (this.epsilon * this.n))
            throw new Error('threshold s is too small');
    
        var res = [];
        for (item in this.list)
            if (this.list[item] >= this.s)
